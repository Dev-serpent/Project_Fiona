...
The `CamComs/esp32payload/` folder is the ESP32-side sender payload template.
...
The ESP32 envelope must match `CamComs.encryption`:
...
`esp32payload.ino` is intentionally a firmware template. Wi-Fi/HTTP and envelope construction are concrete, while the tiny crypto adapter section is isolated because the exact ESP32 crypto library choice may change.

For the current one-way direction, the ESP32 only needs:

- host encryption public key
- ESP32 signing private key

The ESP32 static encryption private key is not needed until the host also encrypts responses back to the ESP32.
